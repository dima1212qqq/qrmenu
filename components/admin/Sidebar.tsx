...
import { ReviewQRModal } from "@/components/ui/ReviewQRModal";
import { AIQRModal } from "@/components/ui/AIQRModal";
...
export function Sidebar({
  ...
  const [showAIQRModal, setShowAIQRModal] = useState(false);
            ...
            {activeMenu && state.organization?.slug && (
              <>
                ...
                <Button
                  variant="secondary"
                  className="w-full"
                  onClick={() => setShowReviewQRModal(true)}
                >
                  <svg className="w-4 h-4 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                  </svg>
                  <span className="truncate">QR Отзывы</span>
                </Button>
                <Button
                  variant="secondary"
                  className="w-full"
                  onClick={() => setShowAIQRModal(true)}
                >
                  <svg className="w-4 h-4 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                  </svg>
                  <span className="truncate">QR AI Помощник</span>
                </Button>
                <Button
                  variant="secondary"
                  className="w-full"
                ...
                >
                  ...
                  <span className="truncate">AI Аналитика</span>
                </Button>
              </>
            )}
          </>
  ...
  return (
    <>
      ...
      {state.organization && (
        <ReviewQRModal
          isOpen={showReviewQRModal}
          onClose={() => setShowReviewQRModal(false)}
          orgSlug={state.organization.slug}
      ...
      {state.organization && (
        <AIQRModal
          isOpen={showAIQRModal}
          onClose={() => setShowAIQRModal(false)}
          orgSlug={state.organization.slug}
          orgName={state.organization.name}
        />
      )}
